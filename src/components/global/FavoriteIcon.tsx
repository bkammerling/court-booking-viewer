...
const FavoriteIcon = ({ slug }: { slug: string }) => {
  const { favorites, toggleFavorite } = useFavorites();

  // Handle favorite button click
  const handleFavoriteClick = (venueSlug: string) => {
    console.log(`Favorited ${venueSlug}`);
    // Add this venue slug to the local storage 
    toggleFavorite(venueSlug);
  }
  
  return (
    <button
      className={`p-1 bg-white rounded-full shadow-md cursor-pointer text-gray-500 transition-transform hover:bg-gray-100 hover:scale-105`}
      onClick={() => handleFavoriteClick(slug)}
      title="Add to favourites"
      aria-label="Favourite"
    >
      <svg
        className={`${favorites.includes(slug) ? 'fill-red-500 animate-expand' : 'text-gray-500'}`}
